...
Перед автозапуском потока необходимо проверять авторизацию пользователя и учитывать настройки приватности (например, запрет автопоказа видео без явного действия пользователя).
