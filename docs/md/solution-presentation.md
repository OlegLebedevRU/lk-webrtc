...
| Аспект | Описание |
|--------|----------|
| **Доступная компонентная база** | ESP32 (Espressif) — массово выпускаемый чип в нижнем ценовом сегменте. STM32F411 — доступный периферийный микроконтроллер. Все компоненты имеют множество поставщиков и не подвержены единой точке отказа в цепи поставок |
...
|-----------|------------|------------|
| **Облачное ядро** | [iot-rpc-rest-app](https://github.com/OlegLebedevRU/iot-rpc-rest-app) | REST API, RPC-ядро, MQTT-брокер, БД, Webhooks |
